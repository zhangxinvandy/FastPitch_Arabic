...
: ${BATCH_SIZE:=4}
: ${GRAD_ACCUMULATION:=4}
: ${OUTPUT_DIR:="./output_arabic_0503"}
: ${LOG_FILE:=$OUTPUT_DIR/nvlog.json}
: ${DATASET_PATH:=./}
: ${TRAIN_FILELIST:=filelists/arabic_audio_pitch_text_train.txt}
: ${VAL_FILELIST:=filelists/arabic_audio_pitch_text_val.txt}
: ${AMP:=true}
: ${SEED:=""}
...
ARGS+=" --benchmark-epochs-num 10"

# Initialize weights with a pre-trained English model
# bash scripts/download_models.sh fastpitch
ARGS+=" --init-from-checkpoint output_arabic_0502/FastPitch_checkpoint_100.pt"
